...
def reply_message():
    ...
    # Use the result of the measurement to reply
    if result.get_counts(qc) == {'0': 1}:
        reply_label.config(text="Reply: " + message)
    else:
        reply_label.config(text="Reply: " + message)
...
reply_button = Button(root, text="Reply", command=reply_message)
reply_button.pack()

reply_label = Label(root)
reply_label.pack()
